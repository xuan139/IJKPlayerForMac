...
- (void)settingWindow{
    ...
    if ([NSWindow instancesRespondToSelector:@selector(setTitleVisibility:)]) {
        ...
        self.view.window.styleMask |= NSWindowStyleMaskFullSizeContentView; //全尺寸
    }
...
//            [self showMediaBar];
            ...
            myDelegate.rate = myDelegate.rate+0.01;
        }
            break;
        case kVK_RightArrow:
        {
...
//            }
            ...
            AppDelegate *myDelegate = (AppDelegate*)[[NSApplication sharedApplication] delegate];
            myDelegate._test = (myDelegate._test == 0) ? 1 : 0;
        }
            break;
        case kVK_UpArrow:
        {
//            [self.playCtl changeVolumeValue:self.player.playbackVolume + self.volumeStep];
            AppDelegate *myDelegate = (AppDelegate*)[[NSApplication sharedApplication] delegate];
            myDelegate.lenswidth = myDelegate.lenswidth+0.01;
...
//            [self.playCtl changeVolumeValue:self.player.playbackVolume - self.volumeStep];
            AppDelegate *myDelegate = (AppDelegate*)[[NSApplication sharedApplication] delegate];
            myDelegate.diff_x = myDelegate.diff_x-0.01;
