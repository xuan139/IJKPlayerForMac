...
void IJK_GLES2_Renderer_free(IJK_GLES2_Renderer *renderer)
{
    ...
    if(renderer->vao_tex != 0)
        glDeleteVertexArrays(1, &renderer->vao_tex);
    if(renderer->vbo_tex != 0)
        glDeleteBuffers(1, &renderer->vbo_tex);
    if(renderer->vbo_texcoords != 0)
        glDeleteBuffers(1, &renderer->vbo_texcoords);
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create_base(const char *fragment_shader_source)
{
    ...
    glValidateProgram(renderer->program);
...
void IJK_GLES2_Renderer_bind_vao(IJK_GLES2_Renderer *renderer){
    glBindVertexArray(renderer->vao_tex);
}

void IJK_GLES2_Renderer_create_buffer(IJK_GLES2_Renderer *renderer){
    ...
    glGenVertexArrays(1, &renderer->vao_tex);
    glBindVertexArray(renderer->vao_tex);
    
    glGenBuffers(1, &renderer->vbo_tex);
    glBindBuffer(GL_ARRAY_BUFFER, renderer->vbo_tex);
    glBufferData(GL_ARRAY_BUFFER, sizeof(texVertices), texVertices, GL_STATIC_DRAW);
    
    glVertexAttribPointer(renderer->av4_position, 4, GL_FLOAT, GL_FALSE, 0, (const GLvoid *)0);
    ...
    glVertexAttribPointer(renderer->av2_texcoord, 2, GL_FLOAT, GL_FALSE, 0, (const GLvoid *)0);
...
IJK_GLES2_Renderer *IJK_GLES2_Renderer_create(SDL_VoutOverlay *overlay)
{
    ...
    IJK_GLES2_checkError("IJK_GLES2_Renderer_create IJK_GLES2_printString");
    ...
    if(renderer)
        IJK_GLES2_Renderer_create_buffer(renderer);
...
GLuint IJK_GLES2_Renderer_get_av4_position(IJK_GLES2_Renderer *renderer){
    
    return renderer->av4_position;
    
}
GLuint IJK_GLES2_Renderer_get_av2_texcoord(IJK_GLES2_Renderer *renderer){
    
    return renderer->av2_texcoord;
...
GLboolean IJK_GLES2_Renderer_renderOverlay(IJK_GLES2_Renderer *renderer, SDL_VoutOverlay *overlay)
{
    ...
    float mobile_res = overlay->screenWidth;
    
    float screenWidth = overlay->screenWidth;
    float screenHeight = overlay->screenHeight;
    
    float diff_x = overlay->diff_x;
    float diff_y = overlay->diff_y;
    float xtimes = overlay->xtimes;
    float ytimes = overlay->ytimes;
    float lenswidth = overlay->lenswidth;
    float rate_y = overlay->rate_y;
    float rate   = overlay->rate;
    
    int   _test = overlay->_test;
    //switch to test
    float   brightness = overlay->brightness;
    float   contrast = overlay->contrast;
    float   saturation = overlay->saturation;
    

    glUniform1fv(glGetUniformLocation(renderer->program, "mobile_res"), 1, &mobile_res);
    glUniform1fv(glGetUniformLocation(renderer->program, "screenWidth"), 1, &screenWidth);
    glUniform1fv(glGetUniformLocation(renderer->program, "screenHeight"), 1, &screenHeight);
    glUniform1fv(glGetUniformLocation(renderer->program, "diff_x"), 1, &diff_x);
    glUniform1fv(glGetUniformLocation(renderer->program, "diff_y"), 1, &diff_y);
    glUniform1fv(glGetUniformLocation(renderer->program, "lenswidth"), 1, &lenswidth);
    glUniform1fv(glGetUniformLocation(renderer->program, "rate_y"), 1, &rate_y);
    glUniform1fv(glGetUniformLocation(renderer->program, "xtimes"), 1, &xtimes);
    glUniform1fv(glGetUniformLocation(renderer->program, "ytimes"), 1, &ytimes);
    glUniform1fv(glGetUniformLocation(renderer->program, "rate"),   1, &rate);
    glUniform1iv(glGetUniformLocation(renderer->program, "_test"),  1, &_test);
    glUniform1fv(glGetUniformLocation(renderer->program, "brightness"), 1, &brightness);
    glUniform1fv(glGetUniformLocation(renderer->program, "contrast"),   1, &contrast);
    glUniform1fv(glGetUniformLocation(renderer->program, "saturation"), 1, &saturation);
    IJK_GLES2_checkError_TRACE("rate");
    

    IJK_GLES2_checkError_TRACE("glDrawArrays1");
    glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);      IJK_GLES2_checkError_TRACE("glDrawArrays2");
